<template>
    <el-select class="tree-select" ref="treeSelect" @visible-change="onPopperVisibleChange" v-model="selectedValue"
        :multiple="multiple" :disabled="disabled" :size="size" :clearable="clearable"
        :popper-class="multiple ? 'tree-select multi-select-tree' : 'tree-select single-select-tree'"
        :collapse-tags="collapseTags" :name="name" :placeholder="placeholder"
        @remove-tag="onRemoveItem">
        <el-option v-for="item in listData" :key="item[nodeKey]" :label="item[props.label || 'label']" :value="item[nodeKey]"></el-option>
    </el-select>
</template>
...
export default {
        ...
        onPopperVisibleChange (isVisible) {
            if (isVisible) {
                this.$nextTick(() => {
                    ...
                    let items = document.getElementsByClassName('el-select-dropdown__item');
                    for (let i = 0; i < items.length; i++) {
                        items[i].style.display = 'none';
                    }
                    let dorpdownList = document.getElementsByClassName('tree-select el-select-dropdown');
                    let dropdownItem = dorpdownList[dorpdownList.length - 1];
                    dropdownItem.getElementsByClassName('el-select-dropdown__list')[0].appendChild(dom);
                    let Tree = Vue.component('ElTree');
                    this.dropdownTree = new Tree({propsData: {
                        ...this.treeProps
                    }});
                ...
                let dorpdownList = document.getElementsByClassName('tree-select el-select-dropdown');
                let dropdownItem = dorpdownList[dorpdownList.length - 1];
                let parent = dropdownItem.getElementsByClassName('el-select-dropdown__list')[0];
        ...
        onRemoveItem (tagValue) {
            if (this.dropdownTree != null) {
                ...
                if (checkedKeys.indexOf(tagValue) !== -1) {
                    this.dropdownTree.setCheckedKeys(checkedKeys.filter((item) => {
    ...
    watch: {
        ...
        selectedValue: {
            handler: function (newVal, oldVal) {
                if (JSON.stringify(newVal) !== JSON.stringify(oldVal) && this.dropdownTree == null) {
